...
def gateway(
    ...
    interactive: bool = typer.Option(
        False, "--interactive", "-i", help="Enable interactive CLI mode"
    ),
):
    ...
    if multi:
        ...
        console.print(f"{__logo__} Starting nanobot gateway in MULTI-AGENT mode on port {port}...")
    ...
    agent = AgentLoop(
        ...
        lsp_config=config.tools.lsp if hasattr(config.tools, 'lsp') else None,
        cron_service=cron,
...
def session_show(
    ...
    console.print(
        f"Created At: [magenta]{session.created_at.strftime('%Y-%m-%d %H:%M:%S')}[/magenta]"
    )
    ...
    if session.messages:
        ...
        for msg in last_messages:
            ...
            console.print(
                f"[[dim]{timestamp}[/dim]] [bold {color}]{role.capitalize()}:[/bold {color}] {display_content}"
            )
    console.print()
